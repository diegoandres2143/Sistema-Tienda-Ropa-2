...
public class ControladorVentas {
    ...
    private final JTable tblHistoricoVentas;          // tabla del historial
    private final JTextField txtHistoricoVentaTotal;  // campo acumulado
    ...
    public ControladorVentas(JComboBox<String> comboCliente,
                          ...
                          JTextField txtTotal,
                          JTable tblDetalles,
                          JTable tblHistoricoVentas,         // nuevo parámetro
                          JTextField txtHistoricoVentaTotal, // nuevo parámetro
                          JTextArea txtAvisos) {
    ...
    this.tblHistoricoVentas = tblHistoricoVentas;           // asignación nueva
    this.txtHistoricoVentaTotal = txtHistoricoVentaTotal;   // asignación nueva
    ...
    public void cargarHistorialVentas() {
        try {
            ...
            DefaultTableModel model = (DefaultTableModel) tblHistoricoVentas.getModel();
            model.setRowCount(0);

            double sumaTotal = 0.0;

            for (Venta venta : ventas) {
                String nombreCliente = obtenerNombreCliente(venta.getClienteCedula());
            ...
            txtHistoricoVentaTotal.setText(String.format("$%.2f", sumaTotal));
            mostrarAviso("Historial actualizado. Ventas: " + ventas.size() 
                    + " | Acumulado: $" + String.format("%.2f", sumaTotal));
        } catch (Exception ex) {
            mostrarAviso("Error al cargar historial: " + ex.getMessage());
        }
    }
    ...
    private String obtenerNombreCliente(String cedula) {
        try {
            ...
            // Si falla, se devuelve la cédula
        }
        return cedula;
    }
}
